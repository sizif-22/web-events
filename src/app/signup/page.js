...
const SignUp = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    profilePicture: "https://firebasestorage.googleapis.com/v0/b/m4-tazkarti.appspot.com/o/profileImgs%2Fdownload.jpeg?alt=media&token=b1c122af-6c6d-4714-a985-7b6e1f006b6e"
  });
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      // Add user with default profile picture first
      await addUser({
      ...
      setLoading(false);
      setStep(2); // Move to profile picture step after successful signup
    } catch (error) {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-200 py-12 px-4">
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle>{step === 1 ? 'Create an account' : 'Add a profile picture'}</CardTitle>
          <CardDescription>
            {step === 1 ? 'Fill in your details to get started' : 'Customize your profile with a photo'}
          </CardDescription>
        </CardHeader>
